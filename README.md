...
[InstaCleanser on the Chrome Web Store](https://chromewebstore.google.com/detail/ecphalengelfdcnhallaepjbjgbcbnbb)
...
**Privacy policy (Chrome Web Store):** Use [`site/privacy.html`](site/privacy.html) as the policy URL (for example `https://troydavis.me/instacleanser/privacy.html`). `homepage_url` in [`manifest.json`](manifest.json) points at the public site; ship a new package version when you change it.
